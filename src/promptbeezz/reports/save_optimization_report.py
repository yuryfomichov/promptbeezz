...
from promptbeezz.schemas import OptimizationResult, TaskSpec
...
async def save_optimization_report(
    ...
    for track in result.all_tracks:
        ...
        best_score = (
            max(track.score_progression)
            if track.score_progression
            else track.final_prompt.rigorous_score
        )
    ...
    if champion_failures:
        ...
        for i, test_result in enumerate(champion_failures, 1):
            ...
            if test_case:
                ...
                lines.append(f"   Score: {test_result.evaluation.overall:.2f}/10\n")
                lines.append(f"   Issue: {test_result.evaluation.reasoning[:100]}...\n")
                lines.append("   (Test case details not found in rigorous tests)\n\n")
    else:
    ...
    if result.original_system_prompt and result.original_system_prompt_test_results:
        ...
        original_failures = [
            test
            for test in result.original_system_prompt_test_results
            if test.evaluation.overall < 7.0
        ]
