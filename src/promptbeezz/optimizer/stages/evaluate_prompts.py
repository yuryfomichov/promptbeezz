...
from promptbeezz.storage import PromptConverter, TestCaseConverter
from promptbeezz.storage.models import Prompt
...
class EvaluatePromptsStage(BaseStage):
    ...
    async def _run_async(self, context: RunContext) -> RunContext:
        ...
        scores_display = [
            f"{getattr(p, score_field):.2f}"
            for p in prompts[:5]
            if getattr(p, score_field) is not None
        ]
        self._print_progress(f"Evaluation complete (scores: {scores_display}...)")
        ...
        if self.stage_name == "quick_filter":
            ...
            if original_db_prompt:
                ...
                if original_pydantic.quick_score is not None:
                    ...
                    rank = (
                        all_scores.index(original_pydantic.quick_score) + 1
                        if original_pydantic.quick_score in all_scores
                        else None
                    )
