...
from promptbeezz.agents.refiner_agent import RefinedPromptOutput, create_refiner_agent
...
from promptbeezz.schemas import PromptCandidate, TaskSpec, WeaknessAnalysis
from promptbeezz.storage import PromptConverter, TestCaseConverter, WeaknessAnalysisConverter
...
class RefinementStage(BaseStage):
    ...
    async def _generate_refinement(
        ...
        Returns:
            Refined prompt text
        """
        if self.config.refiner_client is None:
            raise ValueError("OptimizerConfig.refiner_client is required")
        refiner_client = self.config.refiner_client
        ...
        return self._parse_refined_prompt(await refiner.run())
