...
from promptbeezz.optimizer.stages import (
...
from promptbeezz.schemas import OptimizationResult
...
class PromptOptimizer:
    ...
    def __init__(
        self,
        model_client: BaseConnector | None,
        ...
        self.config = config
        self.config.validate_runtime_dependencies()
        self.model_client = model_client or self.config.connector
        if self.model_client is None:
            raise ValueError(
                "A connector must be provided via argument or OptimizerConfig.connector"
            )
    ...
    async def optimize(self) -> OptimizationResult:
        ...
        try:
            ...
            from promptbeezz.storage.repositories import RunRepository
            ...
            if run_repo is not None and run_id is not None:
                run_repo.fail(run_id=run_id, total_time=time.time() - start_time)
            raise
