...
from promptbeezz.config import LLMConfig, OptimizerConfig, TestDistribution
from promptbeezz.schemas import TaskSpec
from promptbeezz.storage import Database
from tests.helpers import DummyConnector, FakeEvaluator, FakeLLMClient


@pytest.fixture
def test_database():
    """
    Provide a Database instance with in-memory SQLite storage.

    Uses in-memory database for fast, isolated tests with automatic cleanup.
    No files are created on disk, making tests faster and cleaner.
    """
    db = Database(":memory:")
    yield db
    # In-memory databases are automatically cleaned up when connection closes
...
def mock_agents(monkeypatch):
    """
    Backwards-compatible fixture placeholder for legacy tests.
    """
    del monkeypatch


def _attach_fake_runtime(config: OptimizerConfig) -> OptimizerConfig:
    fake_client = FakeLLMClient()
    config.generator_client = fake_client
    config.test_designer_client = fake_client
    config.refiner_client = fake_client
